...
const getsDaysIn = (range: Range) => {
  // For when range is daily (date unused)
  ...
  return (
    ...
    <div
      className={cn(
        ...
        isColumnSecondary?.(index) ? "bg-muted/40" : ""
      )}
      style={
        isColumnSecondary?.(index)
          ? {
              backgroundImage: `repeating-linear-gradient(-45deg, transparent, transparent 4px, hsl(var(--muted) / 0.5) 4px, hsl(var(--muted) / 0.5) 5px)`,
            }
          : undefined
      }
...
export type GanttFeatureItemCardProps = Pick<GanttFeature, "id"> & {
  children?: ReactNode;
  /** Cor do status da task (ex.: #22c55e) — aplica fundo com transparência no card */
  statusColor?: string;
};
  ...
  return (
    <Card
      className={cn(
        ...
        !statusColor && "bg-card border-border"
      )}
      style={
        statusColor
          ? {
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({
  ...
  const barWidthPx = Math.max(Math.round(width), 140);
  const maxExpandPx = 600; // largura máxima ao expandir (permite animação suave)
  ...
  return (
    ...
    >
      <div
        className="pointer-events-auto absolute top-0.5 overflow-hidden hover:z-10 transition-[max-width] duration-300 ease-out"
        style={{
          height: "calc(var(--gantt-row-height) - 4px)",
          width: "max-content",
          minWidth: barWidthPx,
          maxWidth: isBarHovered ? maxExpandPx : barWidthPx,
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- throttle() return type not inferred for deps
